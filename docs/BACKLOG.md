...
| I12 | **把项目搬到用户自己的 GitHub 仓库**（2026-09-25 用户明确要求） | 现在三个 remote 里 `neochron`/`midstream` 都指向**别人**的仓库 `github.com/Elyyyyyyyyxer/Elychron.git`，`upstream` 指官方 `Celechron/Celechron`。**当前 25 个提交一个都没推过**（`main` 本地比远端多 4 个，feature 分支完全没有追踪分支）—— 只活在这台机器上。<br>**步骤**：① 用户在 GitHub 建一个**空**仓库（建议名 `Neochron`，不要初始化 README/.gitignore/LICENSE）；② 我加 remote、并把残缺克隆补成完整克隆（`git fetch --refetch`，否则 push 时要临时去别人的仓库抓 blob）；③ 合并 feature 分支进 `main`（待用户点头，见对话）；④ **push 由用户自己执行**（AGENTS.md 硬约束 1：我绝不 push）。<br>**搬迁后必须同步的文件**（否则功能静默失效）：`lib/worker/fuse.dart` 的 `releaseRepo`、`lib/page/option/credits_page.dart` 的源码链接、`lib/http/calendar_config_parser.dart:130` 的校历 raw 地址、`PRIVACY.md` 的 release API、`README.md`（同时要把 I6 一起做掉，因为公开仓库的门面还是 Elychron） |
