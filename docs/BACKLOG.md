...
| I11 | **签名密钥：现在手机上那个 App 绑在本机的 debug key 上，而这个 key 没有任何备份** | 已核实：`android/key.properties` 不存在 → `build.gradle:87` 退回 `signingConfigs.debug`；本机唯一的密钥是 `C:\Users\Speci\.android\debug.keystore`（2026-09-25 12:23 自动生成，指纹 `6D:B3:75:24:…:B1:FD`），**与你手机上那个 App 的签名逐位相同**。文档里原本要用的正式密钥（`D:\keys\elychron-release.jks`，SHA-256 `b2cc4256…a771c8`）在这台机器上**不存在**。风险与做法见 `docs/RELEASE.md` 的 P0-2 与「四、签名步骤」；**结论只有你能定**（见 2026-09-25 的对话） |
