...
| I11 | **签名密钥：现在没有正式密钥，手机上那个 App 绑在本机自动生成的 debug key 上** | 2026-09-25 核实并**更正过一次**：`docs/RELEASE.md` 里记的那份 keystore（`D:\keys\elychron-release.jks`，SHA-256 `b2cc4256…a771c8`）是**前一位维护者**的东西，用户从没拿到过，也从没发布过 release 签名的包 —— 所以**没有"老用户升级链"需要保护**。现状：`android/key.properties` 不存在 → `build.gradle:87` 退回 `signingConfigs.debug`；本机唯一密钥是 `C:\Users\Speci\.android\debug.keystore`（2026-09-25 12:23 自动生成，指纹 `6D:B3:75:24:…:B1:FD`），与手机上那个 App 的签名逐位相同。<br>**定下来的做法**：生成**自己的**正式密钥（`tools/setup_signing.ps1` 的默认值已改成 Neochron：`D:\keys\neochron-release.jks`、别名 `neochron`、DN `CN=Neochron`），备份两处，然后手机**一次性**「导出数据 → 卸载 → 装新包 → 导入」切过去；此后升级链稳定。（不做也行，但 db 密钥换机器就断，且不适合公开发布） |
