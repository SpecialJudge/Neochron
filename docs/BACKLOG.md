...
| I6 | 仓库自己的 `README.md` / `PRIVACY.md` 仍写 Elychron | 那是**项目**（不是应用）的名字，而且这两个文件里带着真实仓库地址，所以 2026-09-25 改名时按 P0+P1 范围**刻意没动**。要改的话连着 PRIVACY 里的 release API 地址一起看。改不改由你定，不改也没任何功能影响 |
| I7 | **GitHub 仓库名要不要从 Elychron 改成 Neochron** | 一改，**四处代码/文档必须同步**，否则功能会静默失效：① `lib/worker/fuse.dart` 的 `releaseRepo`（更新检查）；② `lib/page/option/credits_page.dart` 的源码链接；③ `lib/http/calendar_config_parser.dart:130` 的校历 raw 地址（改错 = 校历拉不到）；④ `PRIVACY.md`。另外本地两个 remote（`neochron`、`midstream`）的 URL 要 `git remote set-url`。**代价**：旧 release 下载链接会 404，老用户的"检查更新"会失效 —— 得先发一个带新地址的版本才修得回来 |
| I8 | `celechron://` 深链接要不要加 `neochron://` | 现在 manifest 注册的是 `<data android:scheme="celechron"/>`，这是**对外接口**（外部应用/二维码打开本 App），改名时**刻意没动**。想换的话建议**两个并存**（manifest 里再加一条 `<data>`），这样老链接不废、新链接可用 |
| I9 | 桌面端与 iOS 模板工程里的名字（本次按你的选择没做） | `windows/CMakeLists.txt` 的 `BINARY_NAME`、`windows/runner/main.cpp` 的窗口标题、`macos/Runner/Configs/AppInfo.xcconfig` 的 `PRODUCT_NAME`、`ios/Runner/Info.plist` 的 `CFBundleDisplayName`。你只做 Android，等真要做桌面端/iOS 时一起改即可；Android 那两个小组件名（`校园卡`、`最近待办`）是功能名不是应用名，建议一直别动 |
| I10 | 版本号里的 `elychron` 字样（本次按你的决定没动） | 现在是 `version: 1.4.2-elychron.1+10`。下次发版时顺手换成 Neochron（如 `1.5.0-neochron.1+11`）。**注意 `versionCode`（`+` 后面那个）必须不小于已装版本**，否则手机上覆盖安装会被拒 |
| I11 | **签名密钥** ✅ 2026-09-25 已解决（只剩手机上一次性切换） | 2026-09-25 核实并**更正过一次**：`docs/RELEASE.md` 里记的那份 keystore（`D:\keys\elychron-release.jks`，SHA-256 `b2cc4256…a771c8`）是**前一位维护者**的东西，用户从没拿到过，也从没发布过 release 签名的包 —— 所以**没有"老用户升级链"需要保护**。<br>**已做**：`tools/setup_signing.ps1` 默认值改成 Neochron（顺带修掉它在新版环境下跑不起来的老 bug —— 无 BOM 的 UTF-8 被 PS 5.1 按 GBK 读，提交 `600f6e3`/`7265b5f`/`017edad`）；用户在 `D:\keys\neochron-release.jks` 生成了自己的密钥（PKCS12，别名 `neochron`，`CN=Neochron`）；重建后 `apksigner` 读出 SHA-256 `1ecde307…eb31c`、v2 签名有效；新指纹已写进 `docs/RELEASE.md` 与 `README.md`。<br>**剩下**：手机上做一次「导出 → 卸载 → 装新包 → 重新登录 → 导入」（换签名不能覆盖安装），此后升级链永久稳定；以及**把 keystore 与密码各备份两处**（唯一"丢了就永久麻烦"的东西） |
| I12 | ✅ **把项目搬到用户自己的 GitHub 仓库**（2026-09-25 已完成，只剩 push 由用户执行） | 现在三个 remote 里 `neochron`/`midstream` 都指向**别人**的仓库 `github.com/Elyyyyyyyyxer/Elychron.git`，`upstream` 指官方 `Celechron/Celechron`。**当前 25 个提交一个都没推过**（`main` 本地比远端多 4 个，feature 分支完全没有追踪分支）—— 只活在这台机器上。<br>**步骤**：① 用户在 GitHub 建一个**空**仓库（建议名 `Neochron`，不要初始化 README/.gitignore/LICENSE）；② 我加 remote、并把残缺克隆补成完整克隆（`git fetch --refetch`，否则 push 时要临时去别人的仓库抓 blob）；③ 合并 feature 分支进 `main`（待用户点头，见对话）；④ **push 由用户自己执行**（AGENTS.md 硬约束 1：我绝不 push）。<br>**搬迁后必须同步的文件**（否则功能静默失效）：`lib/worker/fuse.dart` 的 `releaseRepo`、`lib/page/option/credits_page.dart` 的源码链接、`lib/http/calendar_config_parser.dart:130` 的校历 raw 地址、`PRIVACY.md` 的 release API、`README.md`（README/PRIVACY 的名称部分已于同日先做掉）。<br>**2026-09-25 已做**：加 remote `mine` = `SpecialJudge/Neochron`；`feat/user-event-edit-page` 与 `feat/rename-neochron` 已快进并入 `main`（main 现含 37 个提交）；7 处链接改成自己的仓库（`fuse.dart` 的 releaseRepo、关于页源码链接、校历镜像④、README 的下载与 Issues、PRIVACY 的 API 与 Issues）；README 顶部那条"还没改完"的提示已删。**剩下的只有 push** |
